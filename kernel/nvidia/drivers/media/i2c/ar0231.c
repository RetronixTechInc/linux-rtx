...
#define AR0231_MIN_GAIN				(1)
#define AR0231_MAX_GAIN				(48)
#define AR0231_DEFAULT_FRAME_LENGTH		(1522)
...
extern int max9295_write_reg_Ser(struct i2c_client *i2c_client,
                                u16 addr, u8 val);
extern int max9296_write_reg_Dser(struct i2c_client *i2c_client,
                                u16 addr, u8 val);
...
static int ar0231_write_table(struct ar0231 *priv,
				const struct index_reg_16 table[])
{
	int ret = 0;
	int i = 0;
	...
	while(table[i].source != 0x00)
	{
		if(table[i].source == 0x06) {
			ret = ar0231_write_reg(priv->s_data, table[i].addr, table[i].val);
			if(0x0010 == table[i].addr)
			msleep(100);
		} else if(table[i].source == 0x05) {
...
{
	struct ar0231 *priv = (struct ar0231 *)tc_dev->priv;
	priv->frame_length = AR0231_DEFAULT_FRAME_LENGTH;
...
static int ar0231_set_exposure(struct tegracam_device *tc_dev, s64 val)
{
	...
	if(AR0231_MIN_FRAME_LENGTH < coarse_time)
	{
...
static int ar0231_set_mode(struct tegracam_device *tc_dev)
{
	...
	err = ar0231_write_table(priv, mode_table[AR0231_MODE_Dser_Ser]);
	...
	usleep_range(3100, 3150);
	ar0231_read_reg(priv->s_data, 0x30ce, &val);

	printk("ar0231_read_reg 0x%x= 0x%x\n", 0x30ce, val);
