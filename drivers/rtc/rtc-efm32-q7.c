...
/* -------------------------------------------------------------------------------------------------------------------------------------- */
/* Keep power for SOC */
static void efm32_q7_keeppower( struct efm32_q7_data *efm32data , int enable )
{
	if ( efm32data )
	{
...
/**/
static void efm32_q7_boot( struct efm32_q7_data *efm32data , int enable )
{
	if ( efm32data )
	{
...
/* Reset MCU */
static void efm32_q7_reset( struct efm32_q7_data *efm32data , int enable )
{
	if ( efm32data )
	{
...
static int efm32_q7_check_checksum( unsigned char *data )
{
	int index , max_index ;
...
static int efm32_q7_read( struct device *dev , void *wdata , uint8_t wlen , void *rdata , uint8_t rlen )
{
	struct i2c_client *client = to_i2c_client( dev ) ;
	struct efm32_q7_data *efm32data ;
	struct i2c_msg msgs[] = {
		{
			.addr = client->addr,
			.flags = 0,
...
/* -------------------------------------------------------------------------------------------------------------------------------------- */
/* EFM32 set status */
static int efm32_q7_status_set( struct device *dev , uint8_t val )
{
	...
	error = efm32_q7_read( dev , ucWrite , WriteLength , ucRead , ReadLength ) ;
	if ( error )
	{
		return error ;
	}
	...
	if ( efm32_q7_check_checksum( ucRead ) )
	{
		switch ( ucRead[1] ) 
		{
...
/* EFM32 get status */
static int efm32_q7_status_get( struct device *dev , uint8_t *val )
{
	unsigned char ucWrite[64] ;
	unsigned char ucRead[64]  ;
...
/* EFM32 set WDOG time */
int efm32_q7_wdog_set( struct device *dev , uint32_t val )
{
	unsigned char ucWrite[64] ;
	unsigned char ucRead[64]  ;
...
int efm32_q7_wdog_set_times( struct device *dev , uint8_t val )
{
	unsigned char ucWrite[64];
	unsigned char ucRead[64];
...
static int efm32_q7_wdog_get( struct device *dev , unsigned char* val )
{
	unsigned char ucWrite[64];
	unsigned char ucRead[64];
...
static int efm32_q7_get_time( struct device *dev , struct rtc_time *tm )
{
	unsigned char ucWrite[64] ;
	unsigned char ucRead[64]  ;
	...
	struct efm32_q7_data *efm32data = i2c_get_clientdata( i2cclient ) ;
...
static int efm32_q7_set_time( struct device *dev, struct rtc_time *tm )
{
	unsigned char ucWrite[64] ;
	unsigned char ucRead[64]  ;
...
static int efm32_q7_get_alarm( struct device *dev , struct rtc_wkalrm *alrm )
{
	unsigned char ucWrite[64] ;
	unsigned char ucRead[64]  ;
...
static int efm32_q7_set_alarm( struct device *dev , struct rtc_wkalrm *alrm )
{
	unsigned char ucWrite[64] ;
	unsigned char ucRead[64]  ;
...
static ssize_t ac_plug_high_show( struct device *dev , struct device_attribute *attr , char *buf )
{
	...
	efm32_q7_reset( efm32data , 1 ) ;
	
	return sprintf(buf, "set_ac_plug:high\n");
}
static DEVICE_ATTR_RO( ac_plug_high ) ;
//static DEVICE_ATTR(ac_plug_high, 0444, ac_plug_high_show, NULL);
...
//static DEVICE_ATTR(startup_finish, 0222, NULL, startup_finish_store);

/* -------------------------------------------------------------------------------------------------------------------------------------- */
/* Open EFM32 device */
static int efm32_q7_rtc_open( struct device *dev )
{
	struct i2c_client *i2cclient = to_i2c_client( dev ) ;
	struct efm32_q7_data *efm32data = i2c_get_clientdata( i2cclient ) ;
...
static void efm32_q7_rtc_release( struct device *dev )
{
	struct i2c_client *i2cclient = to_i2c_client( dev ) ;
	struct efm32_q7_data *efm32data = i2c_get_clientdata( i2cclient ) ;
...
/* -------------------------------------------------------------------------------------------------------------------------------------- */
static int efm32_q7_rtc_ioctl( struct device *dev , unsigned int cmd , unsigned long arg )
{
	struct i2c_client *i2cclient = to_i2c_client( dev ) ;
	struct efm32_q7_data *efm32data = i2c_get_clientdata( i2cclient ) ;
			...
			if( ucWrite[0] == 0x5A && ucWrite[1] == 0xA5 )
			{
				...
				{	
					...
					case 0x01:			//set power on gpio high
						efm32_q7_keeppower( efm32data , 1 ) ;
						return 0;
					case 0x02:			//set power on gpio low
						efm32_q7_keeppower( efm32data , 0 ) ;
						return 0;
					case 0x03:			//set power status
						error = efm32_q7_status_set( dev , ucWrite[3] ) ;
						if( error == 0 )
						{
							return 0 ;
						}
					...
					case 0x04:			//get power status
						{
							...
							error = efm32_q7_status_get( dev , &status ) ;
					...
					case 0x05:			//save MCU RTC
						{
							error = efm32_q7_get_time( dev , &efm32_tm_save ) ;
							if(error == 0)
							{
								return 0;
							}
					...
					case 0x06:			//restore MCU RTC
						{
							error = efm32_q7_set_time( dev , &efm32_tm_save ) ;
							if( error == 0 )
							{
								return 0 ;
							}
...
static const struct rtc_class_ops efm32_q7_rtc_ops = {
	.open 		= efm32_q7_rtc_open,
	.release 	= efm32_q7_rtc_release,
	.ioctl 		= efm32_q7_rtc_ioctl,
	.read_time	= efm32_q7_get_time,
	.set_time	= efm32_q7_set_time,
	/*
	.read_alarm	= efm32_q7_get_alarm,
	.set_alarm	= efm32_q7_set_alarm,
	*/
};

/* -------------------------------------------------------------------------------------------------------------------------------------- */
static void efm32_q7_poweroff(void)
{
	struct efm32_q7_data *efm32data ;
	int error;
	...
	if ( efm32_i2c_client )
	{
		...
		efm32_q7_boot( efm32data , 3 ) ;
		error = efm32_q7_status_set( &efm32_i2c_client->dev , 0 ) ;
	}
}

/* -------------------------------------------------------------------------------------------------------------------------------------- */
static struct i2c_driver efm32_q7_driver ;

static int efm32_q7_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	struct efm32_q7_data *efm32data ;
	...
	efm32data->rtc = devm_rtc_device_register( &client->dev , efm32_q7_driver.driver.name , &efm32_q7_rtc_ops , THIS_MODULE ) ;
	...
	pm_power_off = efm32_q7_poweroff ;
...
static int efm32_q7_remove( struct i2c_client *client )
{
	struct efm32_q7_data *efm32data ;
...
static const struct i2c_device_id efm32_q7_id[] = {
	{ "efm32-i2c", 0 },
	{ }
};
MODULE_DEVICE_TABLE(i2c, efm32_q7_id);

static const struct of_device_id efm32_q7_dt_ids[] = {
	{ .compatible = "rtx,efm32-q7", },
	{ /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, efm32_q7_dt_ids);

static struct i2c_driver efm32_q7_driver = {
	.driver = {
		.name	= "rtc-efm32-i2c",
		.owner	= THIS_MODULE,
		.of_match_table = efm32_q7_dt_ids,
	},
	.probe		= efm32_q7_probe,
	.remove		= efm32_q7_remove,
	.id_table	= efm32_q7_id,
};

module_i2c_driver(efm32_q7_driver);
