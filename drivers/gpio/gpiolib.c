...
static int _gpiod_export_name(struct gpio_desc *desc, bool direction_may_change, const char *name)
{
	unsigned long		flags;
	int			status;
	const char		*ioname = NULL;
	...
	status = sysfs_create_group(&dev->kobj, &gpio_group);
	if (status)
		goto fail_unregister_device;
...
{
	return _gpiod_export_name(gpio_to_desc(gpio), direction_may_change, name);
}
