...
static int tegra_pcie_dw_parse_dt(struct tegra_pcie_dw *pcie)
{
	...
	if (pcie->mode == DW_PCIE_RC_TYPE) {
		...
		pcie->gpio_rst = of_get_named_gpio(np, "nvidia,gpio_rst",0);
		if (gpio_is_valid(pcie->gpio_rst)) {
			ret = devm_gpio_request(pcie->dev, pcie->gpio_rst,"gpio_rst");
			if (ret < 0) {
				if (ret == -EBUSY) {
					dev_err(pcie->dev,"gpio_rst already in use\n");
					pcie->gpio_rst = -EINVAL;
				} else {
					dev_err(pcie->dev,"gpio_rst gpio_request failed %d\n",ret);
					return ret;
				}
			}
			if (gpio_is_valid(pcie->gpio_rst)) {
				ret = gpio_direction_output(pcie->gpio_rst,1);
				if (ret < 0) {
					dev_err(pcie->dev,"%s: gpio_rst gpio_direction_input failed %d\n",__func__, ret);
					return ret;
				}
				udelay(100);
				gpio_direction_output(pcie->gpio_rst, 0);
				udelay(100);
				gpio_direction_output(pcie->gpio_rst, 1);
			}
		}
